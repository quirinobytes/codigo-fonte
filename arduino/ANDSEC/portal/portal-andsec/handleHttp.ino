...
void handleWifi() {
  ...
  if (server.client().localIP() == apIP) {
    server.sendContent(String("<p>You are connected through the Arduino AP: ") + softAP_ssid + "</p>");
  } else {
    server.sendContent(String("<p>Connected through the wifi ssid: ") + ssid + "</p>");
  }
  server.sendContent(
    "\r\n<br />"
  ...
  Serial.println("now you could use this AP as a brigde to connect to bomb wireless network, and defuse it");
  if (n > 0) {
    for (int i = 0; i < n; i++) {
